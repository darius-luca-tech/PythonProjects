...
window = Tk()  
label = Label(window, text = 'Inceput(x, y):') 
startBox = Entry(window) 
label1 = Label(window, text = 'Final(x, y):')
endBox = Entry(window) 
var = IntVar() 
showPath = ttk.Checkbutton(window, text = 'Arata pasii:', onvalue = 1, offvalue = 8, variable = var) 
submit = Button(window, text = 'Incarca', command = onsubmit) 
...
def mousePress(x): 
	...
	g2 = w // (800 // row) 
	acces = grid[g1][g2] 
	if acces != start and acces != end: 
		if acces.obs == False: 
			acces.obs = True 
			acces.show((255, 255, 255), 0) 
...
def main(): 
	...
	if len(openSet) > 0: 
		...
		if current == end: 
			...
			Tk().wm_withdraw() 
			result = messagebox.askokcancel('Program terminat, (Se afiseaza calea cea mai scurta: \n' + str(temp) + 'Se doreste re-executarea programului?') 
			if result == True: 
