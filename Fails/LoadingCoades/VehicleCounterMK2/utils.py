...
def init_logging(to_file = False): 
	...
	formatter = logging.Formatter( 
		fmt = '%(asctime)s.%(msecs)03d %(levelname) - 8s [%(name)s] %(message)', datefmt = '%Y-%m-%d %H:%M:%S') 
	...
	if to_file: 
		handler_file = logging.handlers.RotatingFileHandler("debug.log", maxBytes = 1024 * 1024 * 400, backupCount = 10) 
