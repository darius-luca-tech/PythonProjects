...
def ISS2():
	...
	Procentaj.BarreISS = "Procentajul rotatiei:" + (barre) + str("\n")
	...
	with open("/var/www/html/Orbit.json", "r") as O:
		...
		for i in thisYear:
			...
			if (d0 <= d1 + timedelta(days=90580) and d0 >= d1):  # i is smaller or equal today + 30 days and bigger or equalt today : Next Perihelion
				Procentaj.NewPlutoPerihelion = d0
				d1 = date(Procentaj.current_year, Procentaj.thisMonth, Procentaj.today)
...
def get_planet(nume, d):
	...
	if nume == "Soare":
		...
		return {
			'N' : math.radians(125.1228 - 0.0529538083 * d),
...
motd() 
print("Toate functiile verificate cu succes")
...
if (raspuns == "DA"):
	...
	text = "Am inteles, iti voi oferi optiuni, tu le vei selecta cu ajutorul numerelor de la 1 la 12 si eu ti-oi oferi cat ai spune Saturn \n"
	for x in text:
		print(x, end = '')
		sys.stdout.flush()
		time.sleep(0.1)
